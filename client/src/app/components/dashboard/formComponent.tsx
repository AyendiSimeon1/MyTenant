...
const SubmittedForms = () => {
  ...
  const fetchAgencies = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:3001/api/v1/agencies');
      const agenciesData = response.data;
      const agencyNames = agenciesData.reduce((acc: { [key: string]: string }, agency: any) => {
  ...
  const fetchTemplates = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:3001/api/v1/templates');
      const templatesData = response.data;
      const templateNames = templatesData.reduce((acc: { [key: number]: string }, template: any) => {
  ...
  const handleSendEmail = async (form: Form) => {
    try {
      ...
      await axios.post('http://127.0.0.1:3001/api/v1/agents/send-approval-mail', {
        email: userEmail,
        formId: form.id,
        link: `http://yourwebsite.com/form/${form.id}/payment`
      });
